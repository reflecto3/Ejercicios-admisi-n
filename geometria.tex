...
Dibujemos el trozo de tela del sastre. Punteado era el deseo original, y el de la línea continua fue el que terminó haciendo. 
    ...
    \]
    Es decir, 4 unidades cuadradas menos que el cuadrado original. La respuesta correcta es la B. \hfill $\square$
...
\begin{solution}
    ...
    \]
    Sabemos que $144 = 12^2$ (vale la pena repasar cuáles son los cuadrados perfectos, al menos hasta 15), luego la base del triángulo mide $b=12\cm$. La opción correcta es la D. \hfill $\square$
...
\begin{solution}
    ...
    \]
    Es decir, el precio por pulgada cuadrada de la pizza pequeña es un $25\%$ más que el de la pizza grande. La opción correcta es la A. \hfill $\square$
    
    {\small \itshape
    Aunque no lo parezca, con aumentar un $25\%$ el diámetro de una pizza, el área aumenta en más del $50\%$, de hecho, va a ser $1.25^2 = 1.5625$ veces el área de la pequeña, un $56\%$ más.}
\end{solution}
...
\begin{solution}
    ...
    \end{tikzpicture}
    \end{center}
    Si $x$ es el ancho del terreno, $2x$ es el largo y su área es $288\text{m}^2 = 2x^2$, por lo que $x^2 = 144 \text m^2$. De esta forma $x = 12$ m. El largo del terreno es, por tanto $2x = 24$ m. La opción correcta es la B. \hfill $\square$
\end{solution}
...
\begin{solution}
    ...
    \]
    Por tanto con certeza André obtuvo un área mayor que la de Daniel. \hfill $\square$. 

    {\small \itshape
    Si olvidamos los de $120$ m, y en su lugar denotamos por $x$ la medida obtenida por André, se simplifican los cálculos.
    ...
    \]
    Y con bastantes menos cálculos, obtenemos que la medida que obtuvo Daniel es menor (¿en qué porcentaje?) que la que obtuvo André.} 
\end{solution}

\question En la figura se muestra un círculo inscrito en un cuadrado cuyo perímetro es 32 cm. El área, en centímetros cuadrados, de la región sombreada con gris corresponde a\footnote{\cite{OLCOMAN1-2021}}
\begin{center}
\begin{tikzpicture}
...
\begin{solution}
    ...
    \begin{tikzpicture}
        \draw (-1, -1) rectangle (1, 1);
        \node[below] at (0,-1) {8cm};
        \node[left] at (-1,0) {8cm};
        \draw[->] (0,0) -- node[above] {4cm} (1,0);
        \draw (0,0) circle [radius=1cm];
        \fill[gray] (-1, -1) -- (0, -1) arc[start angle=-90, end angle=-180, radius =1cm] -- cycle;
    \end{tikzpicture}
    \end{center}
    ...
    Por tanto, el área de lo que queda al quitar el círculo al cuadrado es $64-16\pi$ centímetros cuadrados. Esta área está formada por cuatro esquinas \begin{tikzpicture}[scale = 0.2]
    \draw (-1, -1) rectangle (1, 1);
    \draw (0,0) circle [radius=1cm];
    \fill[gray] (-1, -1) -- (0, -1) arc[start angle=-90, end angle=-180, radius =1cm] -- cycle;
...
\begin{solution}
    Sea $x$ la medida del lado de un cuadrado. Contemos cuantos lados de cuadrado hay en el perímetro del pentaminó
    \begin{center}
    \begin{tikzpicture}
        \foreach \x/\y/\pos in {0.5/0/below, 1/0.5/right, 1.5/1/below, 2.5/1/below, 3/1.5/right, 2.5/2/above, 1.5/2/above, 1/2.5/right, 0.5/3/above, 0/2.5/left, 0/1.5/left, 0/0.5/left}
            \node[\pos] at (\x,\y) {$x$};
        \clip (0,0) -- (1,0) -- (1,1) -- (3,1) -- (3,2) -- (1, 2) -- (1,3) -- (0,3) -- cycle;
        \fill[gray] (0,0) rectangle (1,1);
        \fill[gray] (0,2) rectangle (1,3);
    ...
    \end{tikzpicture}
    \end{center}
    Vemos que hay 12, por lo que $12x = 60\cm$ es el perímetro de la figura. Diviendo entre 12 obtenemos $x = 5\cm$. Cada cuadradito tiene lado $x$, por lo que su áres es de $x^2 = 5^2\cmdos = 25\cmdos$. Como el área sombreada se compone de tres cuadraditos, su área es de $3\cdot 25\cmdos = 75\cmdos$. La respuesta correcta es la D. \hfill $\square$
\end{solution}
